...
class Xtal(Sample):
    ...
    def get_index(self):
        ...
        """
        cell_index = self.getCell().get_index()
    ...
    def get_container(self):
        return self.getCell().get_container()

    def get_name(self):
        return "%s%d:%d" % (
            self.getCell().getRowChr(),
            self.getCell().get_index() + 1,
    ...
    def _do_select(self, component):
        ...
        if isinstance(component, Xtal):
            ...
            if len(component > 2):
                pos_x = component[2]
                pos_y = component[3]
            cell = self.get_component_by_address(Cell._getCellAddress(row, column))
    ...
    def _loadData(self, barcode):
        ...
        if processing_plan is None:
            ...
            for x in processing_plan.plate.xtal_list:
                cell = self.get_component_by_address(Cell._getCellAddress(x.row, x.column))
